...
basis_setting = {'type': 'rbf_k', 'nb': 30, 'gamma': 1}
...
# MIP cv (moral)
print("moral size:", len(moral))
Lam = np.linspace(c0 * 1, c0 * 30, 10)

basis_setting = {'type': 'spline', 'n_knots': 2, 'degree': 2, 'include_intercept': False}
MIP_cv_obj = MIP_cv(
    W_train, edges_star, basis_setting, Lam = Lam,
    moral = moral, stab_edges = MIP_pre_process_obj['stab_edges'], 
...
rs = np.array([m, p, "MIP (moral)", edge_diff(edges_star, MIP_cv_obj['edges'], p), MIP_cv_obj['time']])
RS = np.vstack([RS, rs])

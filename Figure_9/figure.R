...
for(data_name in c("asia", "bowling", "rain", "inssmall")) {
  equalvar_data = 
    rbind(
...
df = df %>%
  ...
  mutate(method = factor(method, levels = c("NPVAR", "eqvar_TD", "eqvar_BU", "NoTears", "RESIT", "CCDr", "CAM", "MIP (super)", "MIP (moral)")),
         ...
         data_name = factor(data_name, levels = c("asia", "bowling", "rain", "inssmall")),
         data_name = fct_recode(data_name, "Asia. 8.8" = "asia", "Bowling. 9.11" = "bowling", "Rain. 14.18" = "rain", "Insurance Small. 15.25" = "inssmall")
  )
